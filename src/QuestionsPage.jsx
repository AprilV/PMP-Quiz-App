...
// Function to remove duplicate questions based on question text and merge categories
const removeDuplicates = (questions) => {
  const uniqueQuestions = [];
  const seenQuestions = new Map();

  for (const question of questions) {
    if (!question.categories) question.categories = []; // Ensure categories exist
    ...
    if (!seenQuestions.has(question.question)) {
      ...
      existingQuestion.categories = [
        ...new Set([...existingQuestion.categories, ...question.categories]),
      ];
    }
  }
...
// Function to count questions by category
const countQuestionsByCategory = (questions) => {
  const categoryCounts = {};

  questions.forEach((question) => {
    if (!question.categories) return; // Prevent crashing
...
// Define your questions directly here (or import from a separate file)
const questions = [
...
const QuestionsPage = () => {
  ...
  const filteredQuestions = (uniqueQuestions || []).filter(
    (q) =>
      ...
      q.categories.some(
        (cat) => cat.toLowerCase() === category.replace(/-/g, " ").toLowerCase()
      )
  );
  ...
  if (quizOver) {
    return (
      <div className="quiz-over"> {/* ✅ Centers the quiz over section */}
        <Confetti /> {/* 🎉 Confetti effect */}
  
        <div className="quiz-over-card"> {/* ✅ Keeps only one centered box */}
          <h2 className="quiz-over-title">Quiz Over!</h2>
          <p className="quiz-over-score">Your Score: <strong>{score} / {filteredQuestions.length}</strong></p>
  ...
  console.log(`Rendering - Current Question Index: ${currentQuestionIndex}`);
  console.log("Total Filtered Questions:", filteredQuestions.length);
  console.log(
    "Current Question Object:",
    filteredQuestions[currentQuestionIndex]
  );
  console.log("Category Counts:", countQuestionsByCategory(uniqueQuestions));
  ...
  const handleNext = () => {
    if (currentQuestionIndex < filteredQuestions.length - 1) {
      ...
      setQuizOver(true); // ✅ When last question is reached, finish quiz
    }
    setSelectedOption(null);
    setShowAnswer(false);
    setIsCorrect(null);
            ...
            options.map((option, index) => (
              <li
                ...
                className={`option ${
                  ...
                  showAnswer && selectedOption === index && option !== answer
                    ? "incorrect"
                    : ""
                }`}
